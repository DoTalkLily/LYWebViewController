...
@interface AWEWebViewController ()<SKStoreProductViewControllerDelegate>
{
    BOOL _loading;
    NSURL *_baseURL;
    NSString *_HTMLString;
    NSURLRequest *_request;
    AWESecurityPolicy *_securityPolicy;
...
@interface AWEWebViewController ()
...
@implementation AWEWebViewController
...
- (void)viewDidLoad
{
    ...
    if (_request) {
        ...
        [self loadURL:[NSURL fileURLWithPath:kAWE404NotFoundHTMLPath]];
    }
...
- (void)viewDidLayoutSubviews
{
    ...
    if (_navigationType == AWEWebViewControllerNavigationBarItem) {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (_navigationType == AWEWebViewControllerNavigationToolItem) {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone && _showsToolBar && _navigationType == AWEWebViewControllerNavigationToolItem) {
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation
{
    ...
    if (_navigationType == AWEWebViewControllerNavigationBarItem) [self updateNavigationItems];
}
...
- (BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPopItem:(UINavigationItem *)item
{
    ...
    if ([self.navigationController.topViewController isKindOfClass:[AWEWebViewController class]]) {
        AWEWebViewController* webVC = (AWEWebViewController*)self.navigationController.topViewController;
        if (webVC.webView.canGoBack) {
            ...
            if (webVC.navigationType == AWEWebViewControllerNavigationBarItem && [webVC.navigationItem.leftBarButtonItems containsObject:webVC.navigationCloseBarButtonItem]) {
...
- (UIProgressView *)progressView
{
    ...
    _progressView.awe_hiddenWhenProgressApproachFullSize = YES;
...
- (UIBarButtonItem *)backBarButtonItem
{
    ...
    _backBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"AWEWebViewController.bundle/AWEWebViewControllerBack"]
...
- (UIBarButtonItem *)forwardBarButtonItem
{
    ...
    _forwardBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"AWEWebViewController.bundle/AWEWebViewControllerNext"]
                                                             style:UIBarButtonItemStylePlain
                                                            target:self
...
- (UIBarButtonItem *)navigationBackBarButtonItem
{
    ...
    UIImage* backItemImage = [[[UINavigationBar appearance] backIndicatorImage] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate]?:[[UIImage imageNamed:@"AWEWebViewController.bundle/backItemImage"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    ...
    UIImage* backItemHlImage = newImage?:[[UIImage imageNamed:@"AWEWebViewController.bundle/backItemImage-hl"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    UIButton* backButton = [UIButton buttonWithType:UIButtonTypeSystem];
    NSDictionary *attr = [[UIBarButtonItem appearance] titleTextAttributesForState:UIControlStateNormal];
    if (attr) {
        [backButton setAttributedTitle:[[NSAttributedString alloc] initWithString:AWEWebViewControllerLocalizedString(@"back", @"back") attributes:attr] forState:UIControlStateNormal];
        ...
        [backButton setTitle:AWEWebViewControllerLocalizedString(@"back", @"back") forState:UIControlStateNormal];
...
- (UIBarButtonItem *)navigationCloseBarButtonItem
{
    ...
    if (self.navigationItem.rightBarButtonItem == _doneItem && self.navigationItem.rightBarButtonItem != nil) {
        _navigationCloseItem = [[UIBarButtonItem alloc] initWithTitle:AWEWebViewControllerLocalizedString(@"close", @"close") style:0 target:self action:@selector(doneButtonClicked:)];
    } else {
        _navigationCloseItem = [[UIBarButtonItem alloc] initWithTitle:AWEWebViewControllerLocalizedString(@"close", @"close") style:0 target:self action:@selector(navigationIemHandleClose:)];
    }
...
- (void)didStartLoad
{
    _backgroundLabel.text = AWEWebViewControllerLocalizedString(@"loading", @"Loading");
    self.navigationItem.title = AWEWebViewControllerLocalizedString(@"loading", @"Loading");
...
- (void)didFinishLoad{
    ...
    _backgroundLabel.text = [NSString stringWithFormat:@"%@\"%@\"%@.", AWEWebViewControllerLocalizedString(@"web page",@""), host?:bundle, AWEWebViewControllerLocalizedString(@"provided",@"")];
...
- (void)didFailLoadWithError:(NSError *)error{
    [self loadURL:[NSURL fileURLWithPath:kAWENetworkErrorHTMLPath]];
    _backgroundLabel.text = [NSString stringWithFormat:@"%@%@",AWEWebViewControllerLocalizedString(@"load failed:", nil) , error.localizedDescription];
    self.navigationItem.title = AWEWebViewControllerLocalizedString(@"load failed", nil);
    if (_navigationType == AWEWebViewControllerNavigationBarItem) {
        [self updateNavigationItems];
    }
    if (_navigationType == AWEWebViewControllerNavigationToolItem) {
...
- (void)actionButtonClicked:(id)sender {
    NSArray *activities = @[[AWEWebViewControllerActivitySafari new], [AWEWebViewControllerActivityChrome new]];
...
- (void)webView:(WKWebView *)webView runJavaScriptAlertPanelWithMessage:(NSString *)message initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(void))completionHandler {
    ...
    UIAlertController* alert = [UIAlertController alertControllerWithTitle:host?:AWEWebViewControllerLocalizedString(@"messages", nil) message:message preferredStyle: UIAlertControllerStyleAlert];
    // Init the cancel action.
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:AWEWebViewControllerLocalizedString(@"cancel", @"cancel") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        if (completionHandler != NULL) {
            completionHandler();
        }
    }];
    // Init the ok action.
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:AWEWebViewControllerLocalizedString(@"confirm", @"confirm") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
...
- (void)webView:(WKWebView *)webView runJavaScriptConfirmPanelWithMessage:(NSString *)message initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(BOOL result))completionHandler {
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:AWEWebViewControllerLocalizedString(@"cancel", @"cancel") style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
        [alert dismissViewControllerAnimated:YES completion:NULL];
        if (completionHandler != NULL) {
...
- (void)webView:(WKWebView *)webView runJavaScriptTextInputPanelWithPrompt:(NSString *)prompt defaultText:(nullable NSString *)defaultText initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(NSString * __nullable result))completionHandler {
    NSString *host = webView.URL.host;
    UIAlertController* alert = [UIAlertController alertControllerWithTitle:prompt?:AWEWebViewControllerLocalizedString(@"messages", nil) message:host preferredStyle:UIAlertControllerStyleAlert];
    // Add text field.
    [alert addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = defaultText?:AWEWebViewControllerLocalizedString(@"input", nil);
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if ([navigationAction.request.URL.absoluteString isEqualToString:kAWE404NotFoundURLKey] || [navigationAction.request.URL.absoluteString isEqualToString:kAWENetworkErrorURLKey]) {
...
{
    NSString *host = webView.URL.host;
    UIAlertController* alert = [UIAlertController alertControllerWithTitle:host?:AWEWebViewControllerLocalizedString(@"messages", nil) message:AWEWebViewControllerLocalizedString(@"terminate", nil) preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:AWEWebViewControllerLocalizedString(@"cancel", @"cancel") style:UIAlertActionStyleCancel handler:NULL];
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:AWEWebViewControllerLocalizedString(@"confirm", @"confirm") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        [alert dismissViewControllerAnimated:YES completion:NULL];
    }];
    [alert addAction:cancelAction];
...
#pragma mark - AWEWebViewProgressDelegate
- (void)updateBarItemStatus
{
    if (self.navigationType == AWEWebViewControllerNavigationBarItem) {
        [self updateNavigationItems];
    } else {
        [self updateToolbarItems];
    }
...
- (void)_updateTitleOfWebVC
{
    ...
    self.navigationItem.title = title.length>0 ? title : AWEWebViewControllerLocalizedString(@"browsing the web", @"browsing the web");
}
